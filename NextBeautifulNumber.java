class NextBeautifulNumber {
    public int nextBeautifulNumber(int n) {
        // Predefined list of "balanced" or "beautiful" numbers.
        // A "beautiful" number is one where the count of each digit 
        // equals the digit itself (e.g., 22 has two 2s, 1333 has three 3s, etc.)
        int[] balancedNumbers = {
        ...
        int left = 0;
        int right = balancedNumbers.length - 1;

        // Perform binary search to find the smallest number greater than n
        while (left <= right) {
            int mid = (right + left) / 2;

            // If mid value is less than or equal to n, discard left half
        ...
        // After the loop, 'left' points to the smallest number > n
